...
class ImapMailbox {
	...
	protected $attachmentsRelativeDir;
	...
	public function __construct($imapPath, $login, $password, $attachmentsDir = null, $serverEncoding = 'utf-8') {
		...
		if($attachmentsDir) {
			...
			$this->attachmentsRelativeDir = rtrim($attachmentsDir, '\\/');
		}
	}
	...
	public function getMail($mailId) {
		...
		$mail->seen = strtolower($head->seen);
		...
		if(isset($head->cc)) {
			...
			foreach($head->cc as $cc) {
				if(!empty($cc->mailbox) && !empty($cc->host)) {
					...
					$ccName = isset($cc->personal) ? $this->decodeMimeStr($cc->personal, $this->serverEncoding) : null;
			...
			$mail->ccString = implode(', ', $ccStrings);
		}
	...
	protected function initMailPart(IncomingMail $mail, $partStructure, $partNum) {
		...
		if($attachmentId) {
			...
			if($this->attachmentsDir) {
				...
				$attachment->filePath = $this->attachmentsRelativeDir . DIRECTORY_SEPARATOR . $fileSysName;
				$attachment->fileRealPath = $this->attachmentsDir . DIRECTORY_SEPARATOR . $fileSysName;
				file_put_contents($attachment->fileRealPath, $data);
			}
