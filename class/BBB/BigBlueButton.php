...
class BigBlueButton {
	...
	*@return The url to join the meeting
	*/
	public function createMeetingURL($name, $meetingID, $attendeePW, $moderatorPW, $meetingDuration, $welcomeString, $moderatorWelcomeString, $logoutURL, $SALT, $URL )
	{
		$url_create = $URL."api/create?";
		$voiceBridge = 70000 + rand(0, 9999);

		$params = 'name='.urlencode($name).'&meetingID='.urlencode($meetingID).'&duration='.urlencode($meetingDuration).'&attendeePW='.urlencode($attendeePW).'&moderatorPW='.urlencode($moderatorPW).'&voiceBridge='.$voiceBridge.'&logoutURL='.urlencode($logoutURL);

		if( trim( $welcomeString ) ) $params .= '&welcome='.urlencode($welcomeString);
		
		if( trim( $moderatorWelcomeString ) ) $params .= '&moderatorOnlyMessage='.urlencode($moderatorWelcomeString);
	...
	*/
	public function createMeetingAndGetJoinURL( $name, $meetingID, $welcomeString, $mPW, $aPW, $SALT, $URL, $logoutURL ) {
		...
		if( $xml && $xml->returncode == 'SUCCESS' ) {
			return ( BigBlueButton::joinURL( $meetingID, $name, $mPW, $SALT, $URL ) );
		}	
		else if( $xml ) {
	...
	*/
	public function createMeetingArray($name, $meetingID, $welcomeString, $moderatorWelcomeString, $mPW, $aPW, $meetingDuration, $SALT, $URL, $logoutURL) {

		$xml = bbb_wrap_simplexml_load_file( BigBlueButton::createMeetingURL($name, $meetingID, $aPW, $mPW, $meetingDuration, $welcomeString, $moderatorWelcomeString, $logoutURL, $SALT, $URL) );
