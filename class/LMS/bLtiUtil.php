...
function postLaunchHTML($newparms, $endpoint, $debug=false, $iframeattr=false, $id) {
    global $last_base_string;
    $r = "<div id=\"ltiLaunchFormSubmitArea\">\n";
    if ( $iframeattr ) {
        $r = "<form action=\"".$endpoint."\" name=\"ltiLaunchForm".$id."\" id=\"ltiLaunchForm".$id."\" method=\"post\" target=\"basicltiLaunchFrame".$id."\" encType=\"application/x-www-form-urlencoded\">\n" ;
    } else {
        $r = "<form action=\"".$endpoint."\" name=\"ltiLaunchForm".$id."\" id=\"ltiLaunchForm".$id."\" method=\"post\" encType=\"application/x-www-form-urlencoded\">\n" ;
    }
    ...
    foreach($newparms as $key => $value ) {
        ...
        if ( $key == "ext_submit" ) {
            $r .= "<input type=\"submit\" class=\"sims-submit hide\" name=\"";
        } else {
    ...
    $r .= "</form>\n";
    if ( $iframeattr ) {
        $r .= "<iframe name=\"basicltiLaunchFrame".$id."\"  id=\"basicltiLaunchFrame".$id."\" src=\"\"\n";
