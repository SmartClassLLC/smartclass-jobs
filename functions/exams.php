...
function sinavDersCevapAnahtari($sinavKodu, $dersKodu, $kitapcikTuru)
{
	...
	$key4A = $dbi->getValue(_EXAM_KEYS_, "cevapAnahtari");
    ...
    else
    {
		...
		$dbi->where("sinavKodu", $sinavKodu);
		$key4Booklet = $dbi->getValue(_EXAM_KEYS_, "cevapAnahtari");
...
function sinavDersKitapcikTuru($ogrID, $sinavKodu, $dersKodu)
{
    ...
    $kitapcikTuru = $dbi->getValue(_EXAM_RESULTS_, "kitapcikTuru");
...
function sinavOgrenciDersCevapAnahtari($ogrID, $dersKodu, $sinavKodu, $kitapcikTuru)
{
    ...
    $dbi->where("kitapcikTuru", $kitapcikTuru);
    $dbi->where("ogrID", $ogrID);
    $mcAnswers = $dbi->getValue(_EXAM_RESULTS_, "mcAnswers");
	
	return $mcAnswers;

	//$row = $db->sql_fetchrow($db->sql_query("SELECT `mcAnswers` AS `studentAnswers` FROM "._EXAM_RESULTS_." WHERE `ogrID`='".$ogrID."' AND `kitapcikTuru`='".$kitapcikTuru."' AND `dersKodu`='".$dersKodu."' AND `sinavKodu`='".$sinavKodu."'"));	
}
...
function dersSoruSayisi($sinavTuruKodu, $dersKodu)
{
	...
	//$row = $db->sql_fetchrow($db->sql_query("SELECT `soruSayisi` FROM "._EXAM_TYPE_COURSES_." WHERE `dersBransKodu`='".$dersKodu."' AND `sinavTuruKodu`='".$sinavTuruKodu."'"));
}
...
function dogruCevapSayisi($anahtar, $cevap, $subjectKeyA = "", $studentID = "", $sinifKodu = "", $subjectID = "", $examID = "")
{
	...
	if(is_array($anahtar))
	{
		...
		{
			//add to x booklet key
			$xbookletKey[$key["questionNoX"]] = $key["key"];
			
			//set the student anser
	...
	//xbooklet key
	//@TODO save this somewhere in db
	//ksort($b);
...
function sinifDersOrtalamasi($sinifKodu, $dersKodu, $sinavKodu)
{
    ...
    $sinifOrtalamasi = $dbi->getValue(_EXAM_CLASS_AVERAGES_, "sinifOrtalamasi");
...
function subeDersOrtalamasi($dersKodu, $sinavKodu, $subeKodu)
{
    ...
    $subeOrtalamasi = $dbi->getValue(_EXAM_BRANCH_AVERAGES_, "subeOrtalamasi");
...
function genelDersOrtalamasi($dersKodu, $sinavKodu)
{
    ...
    $dbi->where("dersKodu", $dersKodu);
    $dbi->where("sinavKodu", $sinavKodu);
    $genelOrtalama = $dbi->getValue(_EXAM_GENERAL_AVERAGES_, "genelOrtalama");
...
/* function */
function isExamPublished($examId, $schoolPublish, $subeKodu)
{
    global $dbi, $globalUserFolder, $ySubeKodu;
	...
	if($subeKodu == 0)
	{
		//get publish info based on the setting
		if($schoolPublish == "1") $dbi->where("subeKodu", $ySubeKodu);
		else $dbi->where("subeKodu", "0");
		$dbi->where("sinavKodu", $examId);
		$publishInfo = $dbi->getOne(_EXAM_RESULTS_PUBLISH_, "aktif, yayinTarihi, yayinSaati");
	}
	else
	{
		//get publish info for the school
		$dbi->where("subeKodu", $ySubeKodu);
		$dbi->where("sinavKodu", $examId);
		$publishInfo = $dbi->getOne(_EXAM_RESULTS_PUBLISH_, "aktif, yayinTarihi, yayinSaati");
		
		//as its school exam, school can publish
		$schoolPublish = "1";
	}
	
	//published
	if($publishInfo["aktif"] == "1") $publishReturn = array("examId" => $examId, "published" => "on", "publishDate" => "", "schoolCanPublish" => $schoolPublish);

	//published for later
	else if($publishInfo["aktif"] == "0") $publishReturn = array("examId" => $examId, "published" => "off", "publishDate" => FormatDateNumeric2Local($publishInfo["yayinTarihi"])." ".$publishInfo["yayinSaati"], "schoolCanPublish" => $schoolPublish);

	//unpublished
	else $publishReturn = array("examId" => $examId, "published" => "off", "publishDate" => "", "schoolCanPublish" => $schoolPublish);
